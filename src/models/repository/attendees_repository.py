...
class AttendeesRepository:
  def insert_attendee(self, attendeesInfo: Dict) -> Dict:
    with db_connection_handler as database:
      try:
        ...
        raise HttpConflictError('Particpante já cadastrado!')
  ...
  def get_attendees_by_event_id(self, event_id: str) -> List[Attendees]:
    with db_connection_handler as database:
      attendees = (
        database.session.query(Attendees).outerjoin(CheckIns, CheckIns.attendeeId == Attendees.id).filter(Attendees.event_id == event_id).with_entities(
          Attendees.id, Attendees.name, Attendees.email, CheckIns.created_at.label('checkedInAt'), Attendees.created_at.label('createdAt')
        ).all()
      )
      return attendees